...
let tip = (bill) =>
  bill < 300 && bill > 50 ? bill * (15 / 100) : bill * (20 / 100);
...
if (MarkDetails.calBMI() > johnDetails.calBMI()) {
  console.log(
    `Mark's BMI(${MarkDetails.BMI}) is higher than john's(${johnDetails.BMI})!`
  );
} else {
  console.log(
    `John's BMI(${johnDetails.BMI}) is higher than mark's(${MarkDetails.BMI})!`
  );
}
